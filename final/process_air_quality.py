...
def main():
    # Use relative path assuming raw data folders (2023, 2024) are in the parent directory of the project
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    
    # Output path for final merged data
    output_dir = os.path.join(os.path.dirname(__file__), "data")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    # Process 2023
    print(f"Starting processing for 2023 in {base_dir}...")
    df_2023 = process_year_folder(os.path.join(base_dir, "2023"), "2023")
    if df_2023 is not None:
        df_2023 = df_2023.sort_values("datetime")
        df_2023.to_csv(os.path.join(output_dir, "cleaned_air_quality_2023.csv"), index=False)
    ...
    if df_2024 is not None:
        ...
        df_2024.to_csv(os.path.join(output_dir, "cleaned_air_quality_2024.csv"), index=False)
    ...
    if df_2023 is not None and df_2024 is not None:
        ...
        print(f"No data found in {base_dir} to merge.")
        return
    ...
    output_path = os.path.join(output_dir, "cleaned_air_quality_merged.csv")
